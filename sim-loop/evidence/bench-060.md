...
## 概要
- cron (giemon-sim-bench)、bench-060。
- ホスト負荷: 中負荷〜漸減 (15:53〜15:54 で load 7.16〜8.16 / 5min 9.1〜9.2 /
  15min 12.1〜12.2、ncpu=10)。bench-057〜059 の高負荷 (15min ≈20) から低下継続。
...
- 根拠: /tmp/robotics_test_b060.txt から直接読み取り。
...
- 根拠: /tmp/giemon_test_b060.txt から直接読み取り。

## 2. Seeded 再現実行 (sim-loop 学習ジョブ相当)
- verdict: **pass (軽量範囲)** — bench-002〜059 と同一の軽量 seeded 再現のみ実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (成熟記録 sim-loop/status/maturity.md の「既存決定関数の
  ...
  bench-055〜059 と同一、git grep で src/**/*.clj* と test/**/*.clj* の
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - コマンド: `clojure -M -i /tmp/seed_parity_bench060.clj`
    (bench-059 の seed スクリプトを複製、内容は bench-002〜059 と同一)。
  - 結果: **SEED-PARITY true** (exit 0)。
    :xf/pos [0.035165560086391295 0.0 0.6104766433183229]、:xf/rot も同一。
    bench-002〜059 の :xf/pos 数値と同一 (決定的再現を 59 点確認)。
  - 根拠: /tmp/seed_b060.txt から直接読み取り。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-060 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜060 は高負荷ホスト (load 5〜184) でも同一結果 —
  負荷条件下での同一性を 58 点確認。本回 bench-060 はテスト中 load
  7.16〜8.16 / 5min 9.2 / 15min 12.1 で同一性を維持。
- evidence の差分: 本回以降の追加は bench-060.md のみ (probe/falsify 再実行なし)。

## 4. 役割外 / 省略
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **省略**
  (コード・測定追加を伴う実装側タスク。bench-059 と同じ扱い。
  現負荷 15min ≈12 は ncpu 10 をやや上回る水準だが、テスト・軽量再現は正常完了)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench060.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/giemon_test_b060.txt, /tmp/robotics_test_b060.txt,
  /tmp/seed_b060.txt) 経由で取得して検証した。テスト結果の数字は
  ファイルから直接読み取ったものである。
- /tmp/seed_parity_bench059.clj (bench-059 由来) を /tmp/seed_parity_bench060.clj に
  複製して使用 (内容は bench-002〜059 と同一 — unblob + reconstitute-arm +
  end-effector 2 回照合)。