...
## 概要
- cron (giemon-sim-bench)、bench-055
- ホスト負荷: 開始時 load averages 6.52 / 7.16 / 8.89 (ncpu=10)。
  テスト実行中に負荷が急上昇 (テスト後 52.41 / 31.64 / 19.07、1min は ncpu の 5 倍超)。
  テスト実行と軽量 seeded 再現はいずれも正常終了 (exit 0)。
  長時間シミュレータ実行・probe 再実行は **skipped (load)**。
- 備考: terminal ツールの foreground stdout が空になる障害が継続しているため、
  すべてのコマンド出力を一時ファイル経由で取得した (bench-039〜054 と同じ回避策)。
...
- verdict: **pass (軽量範囲)** — bench-002〜054 と同一の軽量 seeded 再現のみ実施。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench055.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/giemon_test_b055.txt, /tmp/robotics_test_b055.txt, /tmp/b055_seed.txt) 経由で
  取得して検証した。テスト結果の数字はファイルから直接読み取ったものである。
- /tmp/seed_parity_bench054.clj (bench-054 由来) を /tmp/seed_parity_bench055.clj に複製して使用
  (内容は bench-002〜054 と同一 — unblob + reconstitute-arm + end-effector 2 回照合)。
