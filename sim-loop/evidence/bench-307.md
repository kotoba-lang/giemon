...
- HOST LOAD at gate check: 15-min 34.35 (1-min 44.56, 5-min 41.30) / ncpu 10 → **≈3.4x** ≥ 2x gate
- pre-run script snapshot (earlier same window): 15-min 33.76 / ncpu 10 → ≈3.4x。負荷帯は bench-290 (3.9–5.2x) 前後の高位帯。
...
## Git HEADs (measured, unchanged from bench-306)
- robotics: ad99366 (git rev-parse 実測・不変)
- giemon: 8c3c3e6 (git rev-parse 実測・不変)
- in-flight (未コミット): giemon `M sim-loop/status/maturity.md` + `?? sim-loop/evidence/bench-306.md` (bench-306 の記録自体が未 landed)。robotics は clean。src/test 不変。
