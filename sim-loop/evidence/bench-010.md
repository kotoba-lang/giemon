...
## 概要
- 実行日: 2026-09-05 (JST) — cron (giemon-sim-bench)、bench-010
- bench-009 (同 cron 実行で 05:08 記録) の全結果を本 bench で独立再実行により検証した
  (bench-009 記録と完全一致を確認 → bench-009 の記録は正しい)。
- ホスト負荷: load averages 5.51 (1min) / 5.46 (5min) / 6.22 (15min)、ncpu=10 → **低〜中負荷**
...
- verdict: **pass (軽量範囲)** — bench-002〜009 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `PARITY true` (exit 0)、
    ...
    bench-002〜009 の :xf/pos 数値と同一 (決定的再現を 9 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → 002 → 003 → 004 → 005 → 006 → 007 → 008 → 009 → 010 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜010 は高負荷ホスト (load 5〜37) でも同一結果 — 負荷条件下での同一性を 8 点確認。
- evidence の差分: 前回 (bench-009) 以降、evidence への追加・変更なし
  (bench-009 / falsify-006 は本 cron 実行で追加済み。uncommitted は sim-loop/ まま)。
  テスト結果には影響なし。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench010.clj
