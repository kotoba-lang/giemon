...
## 概要
- cron (giemon-sim-bench)、bench-036
- ホスト負荷: load averages 135.07 (1min) / 117.61 (5min) / 94.90 (15min) 開始
  (ncpu=10 → **高負荷**。bench-013 の過去最高 184.45 に次ぐ水準)。テスト実行と
  軽量 seeded 再現は完了 (いずれも正常終了)。長時間シミュレータ実行・probe 再実行は
  **skipped (load)**。NEXT 項目 (sim 受付口の torque 照合不在の入力空間を広げる probe) は
...
- verdict: **pass (軽量範囲)** — bench-002〜035 と同一の軽量 seeded 再現のみ実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (src + test への seed/learn/train grep で該当 0 件 —
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜035 の :xf/pos 数値と同一 (決定的再現を 34 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-036 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜036 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 34 点確認
  (本回 bench-036 は load 117.61 (5min)。bench-013 の 184.45 (過去最高)、bench-029 の 86.79 に
  次ぐ過去 2〜3 番目水準の極度高負荷でも同一)。
- evidence の差分: 本回以降の追加は bench-036.md のみ。
  uncommitted は sim-loop/ ほか (diag.txt, simloop_files_list.txt) のまま。テスト結果には影響なし。

## 4. skipped (load) / 役割外
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **skipped (load)**
  (load 117.61 (5min) は過去最高水準の極度高負荷。テスト・軽量再現は完了したが重い実験は省略)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench036.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/robotics_test036.txt, /tmp/giemon_test036.txt, /tmp/seed_out036.txt) 経由で
  取得して検証した。テスト結果の数字はファイルから直接読み取ったものである。
- /tmp/seed_parity_bench035.clj (bench-035 由来) を /tmp/seed_parity_bench036.clj に複製して使用
  (内容は bench-002〜035 と同一 — unblob + reconstitute-arm + end-effector 2 回照合)。
