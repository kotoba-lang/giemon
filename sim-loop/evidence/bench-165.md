# bench-165 — giemon sim-loop ベンチ (決定的・タイムスタンプなし)

judgement: **unmeasured** (load skip — 実行時 15min 52.81 ≈ 5.3x ncpu=10、2x gate 大幅超過)
load: 1min 71.60 / 5min 56.60 / 15min 52.81, ncpu=10 (uptime 15:07)
host: macOS, 7 users

## テスト
- 未実行 (load gate skip)。15-min load 52.81 は 2×ncpu=20 を大きく上回り、重い
  test スイートの実測を省略。honest に `unmeasured` と記録し、回帰は assert しない
  (基準値据え置き — bench-164/163/162/161 と同方針)。1min 71.60 / 5min 56.60 で
...
- 本 walk は実測 run なしのため新規 stray probe 掃除は行わなかった (bench-160 の
  ...
  within_limits_missing / within_limits_no_caller / parity_arm6) は intact 確認済
  (ls sim-loop/evidence で存在確認)。
