# bench-157 — giemon sim-loop ベンチ (決定的・タイムスタンプなし)

judgement: **unmeasured** (load skip — 15min 38.98 ≈ 3.90x ncpu=10、2x gate 超過)
load: 1min 32.90 / 5min 42.38 / 15min 38.98, ncpu=10 (uptime 12:49)
host: macOS, 8 users

## テスト
- 未実行 (load gate skip)。15-min load 38.98 ≧ 2×ncpu=20 で重い test スイート
  の実測を省略。honest に `unmeasured` と記録し、回帰は assert しない
  (基準値据え置き — bench-155・156 と同方針)。
