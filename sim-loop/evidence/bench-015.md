...
## 概要
- cron (giemon-sim-bench)、bench-015
- ホスト負荷: load averages 21.39 (1min) / 45.41 (5min) / 67.36 (15min) 開始時 →
  実行完了時 38.65 / 42.80 / 63.70、ncpu=10 → **高負荷** (5min は bench-007 の 37.25 を上回る)。
...
- verdict: **pass (軽量範囲)** — bench-002〜014 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  - 対象: `kotoba.giemon.arm/end-effector` (giemon_arm6 fixture —
    fixtures/giemon_arm6/giemon_arm6.edn を unblob で再構成、q=[0.0 0.2 -0.3 0.0 0.5 0.0])
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    :xf/pos [0.035165560086391295 0.0 0.6104766433183229]
    bench-002〜014 の :xf/pos 数値と同一 (決定的再現を 13 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-015 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜015 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 13 点確認
  (本回 bench-015 は load 45.41 (5min)。bench-007 の 37.25 を上回る負荷での同一結果)。
- evidence の差分: 前回 (bench-014) 以降、evidence への追加・変更なし
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench014.clj
# (giemon_arm6 fixture を unblob で再構成し、end-effector を同入力 2 回実行して = で照合
#  → SEED-PARITY true, exit 0)
```

## 補足
- コード修正なし (ベンチ・記録のみ)。
- 決定的記述のみ、タイムスタンプ非依存。
- 本回は terminal ツールの stdout が断続的に空になる障害があったため、
  コマンド出力を一時ファイル (/tmp/bench015_*.log, /tmp/bench015_*_exit.txt) 経由で
