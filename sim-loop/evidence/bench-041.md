...
## 概要
- cron (giemon-sim-bench)、bench-041
- ホスト負荷: load averages 20.04 (1min) / 123.87 (5min) / 91.97 (15min) 開始、
  実行終了時 6.09 / 80.63 / 79.21 (ncpu=10 → 高負荷帯)。テスト実行と軽量 seeded 再現、
  H16 probe (falsify-014 系) の再実行を実施。
- 備考: terminal ツールの foreground stdout が空になる障害が継続しており、
  すべてのコマンド出力を一時ファイル経由で取得した (bench-039/040 と同じ回避策)。
...
- verdict: **pass (軽量範囲)** — bench-002〜040 と同一の軽量 seeded 再現を実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (status/maturity.md L0 記載どおり。本回も src の .clj への
  seed/learn/train grep 該当ファイル数 0 を再確認)。L1 昇格条件は未達、L0 定着を継続。
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜040 の :xf/pos 数値と同一 (決定的再現を 39 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-041 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜041 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 39 点確認
  (本回 bench-041 は load 123.87 (5min) 開始・80.63 終了の高負荷時。bench-013 の 184.45 が過去最高)。
- evidence の差分: 本回は bench-041.md の追加のみ。uncommitted は sim-loop/ ほか
  (diag.txt, simloop_files_list.txt) のまま変化なし。テスト結果に影響なし。

## 4. H16 probe 再実行 (falsify-014 系)
- probe: `probe_gate_input_space_widening.py` (bench-040 と同一、差し替えなし)。
...
- 決定性: 2 回実行 diff **0 行** (EXIT 0/0)、GATE-WIDENING-DETERMINISM-FAILS 0。
  bench-040 の実測結果と同一 (verdict survived — H16 は未反証のまま残存)。

## 5. skipped (load) / 役割外
- skipped (load) なし (テスト・軽量 seeded 再現・probe 再実行は完了)。
  ただし本回は高負荷時のため重い実験 (フル学習ジョブ相当) は対象外 —
  学習ジョブ自体が未実装 (L0) のため実施項目は軽量範囲のみ。
- コード修正なし (ベンチ・記録のみ)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench041.clj
...
clojure -M -e "$(cat /tmp/h16.clj)" > /tmp/h16r1.txt 2>&1
clojure -M -e "$(cat /tmp/h16.clj)" > /tmp/h16r2.txt 2>&1
diff /tmp/h16r1.txt /tmp/h16r2.txt   # 0 行差 (決定的)
```

## 補足
- 決定的記述のみ、タイムスタンプ非依存。
- /tmp/seed_parity_bench040.clj を /tmp/seed_parity_bench041.clj に複製して使用
  (内容は bench-002〜040 と同一)。
