...
- HOST LOAD at gate check: 15-min 44.09 (1-min 48.92, 5-min 47.10) / ncpu 10 → **≈4.4x** ≥ 2x gate
- pre-run script snapshot (earlier same window): 15-min 44.95 / ncpu 10 → ≈4.5x。負荷帯は bench-311 (≈4.0x) と同水準の gate 超過継続。
...
## Git HEADs (measured, unchanged from bench-311)
- robotics: ad99366 (git rev-parse 実測・不変)
- giemon: 8c3c3e6 (git rev-parse 実測・不変)
- in-flight (未コミット): giemon `M sim-loop/status/maturity.md` + `?? sim-loop/evidence/bench-306.md`〜`?? sim-loop/evidence/bench-312.md` (bench-306〜312 の記録自体が未 landed)。robotics は clean。src/test 不変。
...
- runner status: clojure runner silent-zero (0/0/0 RC=0、bench-244〜306 実測分 46 連続、falsify-069 根因確定: JVM require が .cljk をロード不能) は本 run でも未検証 (load skip、bench-307〜312 で 6 連続 skip)。test-runner 修復が最優先のまま変化なし。
