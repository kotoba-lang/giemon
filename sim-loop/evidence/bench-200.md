# bench-200 — measured (実測完走, 負荷 ~7.4×)

判定: **measured** — HOST LOAD ~7.4× ncpu ( hw.ncpu=10) と Load gate (~2×) を大きく超える高負荷帯だったが、backend 応答で /tmp redirect + read_file workaround により test スイート実測完走 (robotics ・ giemon 両方、exit 0 両方)。per-project 出力ファイル (/tmp/b_rob.txt, /tmp/b_gie.txt) 完走 fixture あり。

## 環境
- HOST LOAD: 実行中 1-min 66.75 / 5-min 81.34 / 15-min 73.59 (→返 ≈7.4×)、up: 3 days 18h、6 users
- Load gate (~2×) 大幅超過帯だが、bench-199 (~4.4×)、bench-198 (~3.1×) と同様に backend 応答で実測取得可能だったため skip せず measured 記録。
...
## 回帰
- **なし (measured assert)** — 両スイートが基準値 ( bench-066 確定 / bench-199 最新実測) と完全一致 (robotics 14/50/0、giemon 46/115/0、両者 exit 0)、HEAD 両方不変・tracked diff 空。回帰なし。
