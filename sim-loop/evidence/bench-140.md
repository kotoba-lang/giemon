# bench-140 — unmeasured ( HOST LOAD gate 超過で skip )

状態: HOST LOAD 15min 25.80 ( 8:04 実測: 1min 25.01、5min 29.18、15min 25.80) が ncpu=10 に対し ≈ 2.6× ( Load gate( 15min ≥ ≈2×ncpu =20) を超過。全窓( 1min・5min・15min) が gate 超過 ( >20) で応答不能リスク高。重い test スイート実行は省略 ( honest 据え置き)。

- uptime(8:04): `1min 25.01、5min 29.18、15min 25.80 — 15min が gate ( ≥20) 超過。実行不可と判断。
