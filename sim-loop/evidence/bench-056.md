...
## 概要
- cron (giemon-sim-bench)、bench-056
- ホスト負荷: 本回も高負荷 (pre-run 時 load 180.40 / 133.50 / 98.56、
  テスト後に 92.02 / 118.06 / 105.57。ncpu=10)。テスト実行と軽量 seeded 再現は
  いずれも正常終了 (exit 0)。
- 長時間シミュレータ実行・probe 再実行は **skipped (load)**。
- 備考: terminal ツールの foreground stdout が空になる障害が継続しているため、
  すべてのコマンド出力を一時ファイル (/tmp/*.txt) 経由で取得した
  (bench-039〜055 と同じ回避策)。
...
- verdict: **pass (軽量範囲)** — bench-002〜055 と同一の軽量 seeded 再現のみ実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (成熟記録 status/maturity.md の「既存決定関数の 2 回実行一致に
  限る」減点事由は解消されず)。本回も src への seed/learn/train 系変更は確認されず
  (git diff --stat 空、git log 直近 d0d3cb4 は bench-055 と同一)。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜055 の :xf/pos 数値と同一 (決定的再現を 55 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-056 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜056 は高負荷ホスト (load 5〜184) でも同一結果 —
  負荷条件下での同一性を 54 点確認。本回 bench-056 はテスト前後で
  load 92〜180 を観測しつつ同一性を維持 (bench-013 の 184.45 が load 1min 過去最高)。
- evidence の差分: 本回以降の追加は bench-056.md のみ。
  uncommitted は sim-loop/ ほか (diag.txt, simloop_files_list.txt, statout.txt) のまま。
  テスト結果には影響なし。

## 4. skipped (load) / 役割外
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **skipped (load)**
  (ホスト load が 1min 92〜180 = ncpu の 9 倍超の高負荷。テスト・軽量再現以外の
  重い実験は省略)。
- OPEN の赤 (URDF parse parity 自動検証不在、gate↔arm torque 照合未接続、
  sim 受付口 torque 照合入力空間拡大 probe、BOM by-name last-wins false-pass の
  実 fixture 到達性 H20、H22 torque-headroom `:joint/limit` 欠落面 probe) はいずれも
  測定コード追加を伴い、本 bot はコードを修正しないため本回も skipped (load)。
  実装側タスクとして継続。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench056.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/giemon_test_b056.txt, /tmp/robotics_test_b056.txt,
  /tmp/b056_seed.txt) 経由で取得して検証した。テスト結果の数字は
  ファイルから直接読み取ったものである。
- /tmp/seed_parity_bench055.clj (bench-055 由来) を /tmp/seed_parity_bench056.clj に
  複製して使用 (内容は bench-002〜055 と同一 — unblob + reconstitute-arm +
  end-effector 2 回照合)。