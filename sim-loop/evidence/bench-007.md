...
## 概要
- 実行日: 2026-09-05 (JST) — cron (giemon-sim-bench)、bench-007
...
- verdict: **pass (軽量範囲)** — HOST LOAD 極度高負荷のため重い実験は skipped (load)、
  bench-002〜006 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `PARITY true` (exit 0)、
    ...
    bench-002〜006 の :xf/pos 数値と同一 (決定的再現を 6 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → 002 → 003 → 004 → 005 → 006 → 007 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- evidence の差分: 前回 (bench-006) 以降、falsify-005 (probe_j2_ramp_family.py —
  falsify-004 の peak 破れ k≈1.04x は probe の慣性トルク次元誤り I·α² の帰結、
  正しい I·α 形式では k≈11.74x に戻る) が追加。テスト結果には影響なし。
  本 bench は falsify-005 の再計測は skipped (load)。
