...
## 概要
- cron (giemon-sim-bench)、bench-028
- ホスト負荷: load averages 15.81 (1min) / 18.32 (5min) / 26.47 (15min) 開始
  (ncpu=10 → **中負荷**)。テスト実行と軽量 seeded 再現は完了 (いずれも正常終了)。
...
- verdict: **pass (軽量範囲)** — bench-002〜027 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜027 の :xf/pos 数値と同一 (決定的再現を 26 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-028 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜028 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 26 点確認
  (本回 bench-028 は load 18.32 (5min)。bench-021 の 12.07、bench-022 の 11.68 より高いが
  bench-019 の 22.65、bench-020 の 22.09、bench-026 の 23.70 より低水準)。
- evidence の差分: bench-027.md が前回実行で追加済み。本回以降の追加は bench-028.md のみ。
  uncommitted は sim-loop/ ほか (diag.txt, simloop_files_list.txt) のまま。テスト結果には影響なし。

## 4. skipped (load) / 役割外
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **skipped (load)**
  (load 18.32 (5min) はテスト・軽量再現は可能だが重い実験は省略する水準)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench028.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/giemon_test.txt, /tmp/robotics_test.txt, /tmp/seed_out028.txt) 経由で
  取得して検証した。テスト結果の数字はファイルから直接読み取ったものである。
- /tmp/seed_parity_bench027.clj を /tmp/seed_parity_bench028.clj に複製して使用
  (内容は bench-024〜027 と同一 — unblob + reconstitute-arm + end-effector 2 回照合)。
