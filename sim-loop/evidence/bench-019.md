...
## 概要
- cron (giemon-sim-bench)、bench-019
- ホスト負荷: load averages 22.78 (1min) / 22.65 (5min) / 25.39 (15min) 開始、ncpu=10 →
...
- verdict: **pass (軽量範囲)** — bench-002〜018 と同一の軽量 seeded 再現のみ実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (src + test の .clj への learn/train/seed grep で該当なし — 本回機械再確認)。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜018 の :xf/pos 数値と同一 (決定的再現を 17 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-019 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜019 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 17 点確認
  (本回 bench-019 は load 22.65 (5min)。bench-017 の 20.46 と同程度、bench-007 の 37.25 を下回る)。
- evidence の差分: 前回 (bench-018) 以降、evidence への追加・変更なし
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench019.clj
...
- 本回も terminal ツールの stdout が断続的に空になる障害があり、コマンド出力を
  一時ファイル (/tmp/giemon_test_019.txt, /tmp/robotics_test_019.txt, /tmp/bench019_host.txt,
  /tmp/bench019_parity.log) 経由で取得して検証した。テスト結果の数字はファイルから
  直接読み取ったものである。
