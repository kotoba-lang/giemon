...
- **理由**: HOST LOAD 15:49 JST = 16.14 / 25.81 / 30.23 (1/5/15min) — 継続的な load 超過。加えて実行バックエンド (terminal / search / sandbox stat) 応答不能: terminal は empty stdout (exit 0) を返し、search_files は「could not stat ... sandbox」 を返した。重い実験 (kbb -M:test、seeded 再現 2 回走行) は省略。
- **再現コマンド**: —
