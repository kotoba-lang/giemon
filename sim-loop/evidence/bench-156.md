# bench-156 — giemon sim-loop ベンチ (決定的・タイムスタンプなし)

judgement: **unmeasured** (load skip — 15min 32.56 ≈ 3.26x ncpu=10、2x gate 超過)
load: 1min 17.87 / 5min 28.04 / 15min 32.56, ncpu=10 (uptime 12:35)
host: macOS, 8 users

## テスト
- 未実行 (load gate skip)。15-min load 32.56 ≧ 2×ncpu=20 で重い test スイート
  の実測を省略。honest に `unmeasured` と記録し、回帰は assert しない
  (基準値据え置き — bench-099〜101・bench-155 と同方針)。
...
- 基準値 (robotics 14/50/0、giemon 46/115/0) に対し本 walk は unmeasured のため
  ...
  = 基準値と同一)。git status: giemon は `?? sim-loop/` 未追跡のみ、tracked
  変更なし → コード変更なし。
