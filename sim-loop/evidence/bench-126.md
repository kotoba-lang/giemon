# bench-126 — skipped (load)

状態: HOST LOAD 超過 (15min ≥ 2×ncpu≈20) で重い test 実行は省略。正直に unmeasured 記録。基準値据え置き。



- uptime ( 4:36〜4:38 実測): 15min = 56.46 → 58.67 (ncpu=10 に対し 約 5.6×〜5.9×)。1min 92〜98、5min 65〜70 — LOAD gate (15min ≥ ~2×ncpu≈20) を大きく超過し継続高止まり、
...
- test スイート (robotics / giemon の `clojure -M:test`): **実行せず** — load 超過帯のため省略 (bench-099〜101、bench-124〜125 と同方針)。fg 実行継続せず。
