...
## 概要
- cron (giemon-sim-bench)、bench-047
- ホスト負荷: 開始時 load averages 34.16 / 75.83 / 79.96、再現実行終了時
  27.21 / 58.04 / 72.20 (高負荷帯)。テスト実行と軽量 seeded 再現を実施。
- 備考: terminal ツールの foreground stdout が空になる障害が継続しており、
  すべてのコマンド出力を一時ファイル経由で取得した (bench-039〜046 と同じ回避策)。
...
- verdict: **pass (軽量範囲)** — bench-002〜046 と同一の軽量 seeded 再現を実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: **SEED-PARITY true × 4 姿勢 / POSTURES-MEASURED 4 (exit 0)**、
    ...
    - bench-q0    → POS [0.035165560086391295 0.0 0.6104766433183229]
      (bench-002〜046 の従来数値と同一 — 回帰なし)
  - 2 回実行 diff **0 行** (diff exit 0 — EXIT ラベル行含め完全一致)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-047 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜047 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 45 点確認
  (本回 bench-047 は load 75.83 (5min) 開始・58.04 終了。bench-013 の 184.45 が過去最高)。
- evidence の差分: 本回は bench-047.md の追加のみ。uncommitted は sim-loop/ ほか
