...
## 概要
- cron (giemon-sim-bench)、bench-053
- ホスト負荷: 開始時 load averages 17.46 / 13.93 / 14.08 (ncpu=10)。
  bench-052 開始時 (51/68/67) より低く、テスト実行と軽量 seeded 再現は完了 (いずれも正常終了)。
  長時間シミュレータ実行・probe 再実行は **skipped (load)**。
- 備考: terminal ツールの foreground stdout が空になる障害が継続しているため、
  すべてのコマンド出力を一時ファイル経由で取得した (bench-039〜052 と同じ回避策)。
...
- verdict: **pass (軽量範囲)** — bench-002〜052 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜052 の :xf/pos 数値と同一 (決定的再現を 52 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-053 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜053 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 51 点確認
  (本回 bench-053 は load 17.46 (1min) / 13.93 (5min) 開始。bench-013 の 184.45 が過去最高)。
- evidence の差分: 本回以降の追加は bench-053.md のみ。
  uncommitted は sim-loop/ ほか (diag.txt, simloop_files_list.txt, statout.txt) のまま。
  テスト結果には影響なし。

## 4. skipped (load) / 役割外
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **skipped (load)**
  (load 13.93 (5min) は ncpu=10 の 1.4 倍。テスト・軽量再現以外の重い実験は省略)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench052.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/b053_robotics.txt, /tmp/b053_giemon.txt, /tmp/b053_seed.txt) 経由で
  取得して検証した。テスト結果の数字はファイルから直接読み取ったものである。
- /tmp/seed_parity_bench039.clj (bench-039 由来) を /tmp/seed_parity_bench052.clj に複製して使用
  (内容は bench-002〜052 と同一 — unblob + reconstitute-arm + end-effector 2 回照合)。
