...
判定: **measured (完走)** — HOST LOAD が ncpu=10 の ~1.1× (15min ~10.98) で kbb -M:test 実測完走。robotics・giemon とも基準値一致、回帰なし。（実行後 uptime は 15.17/11.22/11.30 まで上昇したが完走後に計測。）
...
##再現コマンド
- 実行: `cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/robotics && kbb -M:test > /tmp/b_rob.txt 2>&1`; `cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && kbb -M:test > /tmp/b_gie.txt 2>&1` —完走 (measured,回帰なし)。/tmp/b_rob.txt / /tmp/b_gie.txt に実測保存。頃 2:35。
