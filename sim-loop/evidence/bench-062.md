...
## 概要
- cron (giemon-sim-bench)、bench-062。
- ホスト負荷: 中負荷 (16:20 で load 12.18 / 5min 10.27 / 15min 9.75、ncpu=10)。
  15min ≈10 前後で bench-060/061 と同等の中負荷帯に維持。テスト実行と軽量
  seeded 再現は正常終了 (exit 0)。
...
- 根拠: /tmp/robotics_test_b062.txt から直接読み取り。
...
- 根拠: /tmp/giemon_test_b062.txt から直接読み取り。

## 2. Seeded 再現実行 (sim-loop 学習ジョブ相当)
- verdict: **pass (軽量範囲)** — bench-002〜061 と同一の軽量 seeded 再現のみ実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (成熟記録 sim-loop/status/maturity.md の「既存決定関数の
  ...
  bench-055〜061 と同一、git grep で src/**/*.clj* と test/**/*.clj* の
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - コマンド: `clojure -M -i /tmp/seed_parity_bench062.clj`
    (bench-061 の seed スクリプトを複製、内容は bench-002〜061 と同一)。
  - 結果: **SEED-PARITY true** (exit 0)。
    :xf/pos [0.035165560086391295 0.0 0.6104766433183229]、:xf/rot も同一。
    bench-002〜061 の :xf/pos 数値と同一 (決定的再現を 61 点確認)。
  - 根拠: /tmp/seed_b062.txt から直接読み取り。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-062 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜062 は高/中負荷ホスト (load 5〜184) でも同一結果 —
  負荷条件下での同一性を 60 点確認。本回 bench-062 はテスト時 load
  12.18 / 5min 10.27 / 15min 9.75 で同一性を維持。
- evidence の差分: 本回以降の追加は bench-062.md のみ (probe/falsify 再実行なし)。

## 4. 役割外 / 省略
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **省略**
  (コード・測定追加を伴う実装側タスク。bench-059〜061 と同じ扱い。
  現負荷 15min ≈9.75 は ncpu 10 と同水準で、テスト・軽量再現は正常完了)。
- OPEN の赤 (URDF parse parity 自動検証不在、gate↔arm torque 照合未接続、
  H19/H20/H21/H23 潜在修理点、NEXT H24 within-limits? の limit 丸ごと欠落面
  probe) はいずれも測定コード追加または fixture/コア修正を伴い、本 bot は
  コードを修正しないため本回も省略。実装側タスクとして継続。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench062.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/giemon_test_b062.txt, /tmp/robotics_test_b062.txt,
  /tmp/seed_b062.txt) 経由で取得して検証した。テスト結果の数字は
  ファイルから直接読み取ったものである。
- /tmp/seed_parity_bench061.clj (bench-061 由来) を /tmp/seed_parity_bench062.clj に
  複製して使用 (内容は bench-002〜061 と同一 — unblob + reconstitute-arm +
  end-effector 2 回照合)。
```