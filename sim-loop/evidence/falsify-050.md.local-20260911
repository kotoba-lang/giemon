# falsify-050 — H51 (FK guard repair が着手され、FK 経路は越境 angles を検査するか)

- 連続番号: 050 (falsify-049 の後続)
- 日次: 260908
- 仮説 (H51, NEXT 継続発行 — falsify-034/036/039/040/041/042/043/044/045/046/047/048/049 の 14 連続 refuted を受けた再検証):
...
- 実測 (決定的、純静的読取 — HOST LOAD 超過帯 + terminal backend が素の stdout を swallow し
  redirect 経由のみ応答のため REPL・test 計数は unmeasured、数字捏造ゼロ):
  - **`within-limits?` 定義/呼出分布 (grep src/ test/ 実測):** arm.cljc L15-20 定義・arm.cljc L28
    docstring 言及・arm_test.cljc L28-30 単体テストのみの計 5 ヒット (3 箇所)。FK 経路
  ...
  - 測定時 HOST LOAD: 本 walk 冒頭 18:04 実測 28.90/31.99/39.48 (1min/5min/15min、≈2.9–3.9×
    ncpu=10)。Load gate (15min ≥ 2×ncpu=20) を超過 + terminal backend 応答不能 (素のコマンド
    出力が逐次 swallow、redirect 経由で取得) — 重い test 実行は省略し unmeasured (honest
    据え置き)。
- verdict: **refuted** — FK guard repair は依然未着手 (15 連続)。`within-limits?` は定義・doc・
  ...
  **0 回**、L38 silent zero-fill (`angle (or (first angles) 0.0)`) 不変、arm_test L20-22 が零埋め
  ...
  返却 (docstring L27-29 自白)。falsify-034/036/039/040/041/042/043/044/045/046/047/048/049
...
- 再現手順 (全て静的読取、実行不要):
  ...
  read src/kotoba/giemon/arm.cljc L31-41                  # forward-kinematics loop、L38 `angle (or (first angles) 0.0)`、limit 検査なし
  read test/kotoba/giemon/arm_test.cljc L20-22            # zero-fill 緑 assertion 無変更: (= (fk [0.0 0.0]) (fk []))
  grep -n 'within-limits\|:joint/limit\|forward-kinematics\|end-effector' src/kotoba/giemon/governor.cljc  # → 0 行 (無接続)
  ```
- 検証内訳 (1 仮説・1 実測判定): 1 仮説 (H51) / 測定 1 (純静的読取 — within-limits? grep 分布
  (FK 内呼出 0) + L38 zero-fill 不変 + arm_test L20-22 緑固定 + governor 接続 0 + HEAD/tracked
  diff の 5 観測点) / 判定 refuted (未着手)。
- コアへの 1 行: FK guard repair は 15 連続で未着手のまま (`within-limits?` は定義・単体テストのみ
  ...
  実装・変更なし (HOST LOAD 28.90/31.99/39.48 ≈2.9–3.9× 超過帯で重い test 実行を省略し
  unmeasured 明記)。