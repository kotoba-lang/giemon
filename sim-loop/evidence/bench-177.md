# bench-177 — giemon sim-loop bench (日次・決定的・タイムスタンプなし)

judgement: **measured** — 15-min 負荷帯 ≈ 10× ncpu (実行前 99.81 → 終了後 100.31)
の Load gate 大幅超過帯だったが、実行バックエンドは応答し、両 test スイート実測は
完走、基準値 (bench-066 確定 / bench-176 実測系) と完全一致。回帰なし。

## 実行環境
- git HEAD: giemon `d0d3cb4`、robotics `9459ca0` — 基準値 (bench-066) と同一、不変。
- git diff: 追跡ファイル変更なし (empty)。git status は `?? sim-loop/` のみ未追跡 (bench-176 と同構成)。コード変更なし。
- HOST LOAD: 実行開始前 (pre-run script) 122.10 / 136.88 / 99.74、終了後 113.28 / 132.12 / 100.31
  (15min 99.74→100.31 ≈ 10× ncpu=10)。Load gate (15min≥2×ncpu ≈ 20) に対し大幅超過帯。
- 実行バックエンド: terminal 直接 stdout は空のまま (既知 pitfall) だが `/tmp` redirect +
  ...
  bench-108・167・168 前例に従う)。

## テスト（clojure -M:test 実測、/tmp redirect + read_file、一意ファイル b177_*.txt）
...
再現対象の学習ジョブが存在しないため、seeded 再現の本測定は行わない (bench-102〜108・167〜176 と同方針)。
...
コアは falsify-050・bench-176 計測時と同一ソース。純静的再確認: `within-limits?` のヒットは
arm.cljc L15 (def)・L28 (doc)・arm_test.cljc L29-30 (単体テスト) の計 3 箇所のみで、FK 経路
(forward-kinematics / end-effector) 内呼出は 0 回 (falsify-049 〜 bench-176 と同じ、FK guard
...
- 本例は 15-min 帯域 ≈ 10× ncpu の Load gate 大幅超過帯だったが、backend 応答・実測完走・
