# bench-30 (2026-09-04 16:19 JST, cron)
...
- 前回 (bench-29) 比: 同値。回帰なし。

## seeded 再現
- `grep -rl seed src` → 0 件。seeded 学習ジョブ対象不在のため **not-run** (bench-14〜29 と同値)。

## 負荷
- load averages: 31.30 / 47.06 / 43.79 (コア 10)。高負荷のため重い追加実験は **skipped (load)**。
  軽量テスト実行のみ実施。
