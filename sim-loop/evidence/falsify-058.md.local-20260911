# falsify-058 — H59 (FK guard repair が配線されたか: `within-limits?` が FK 経路内から呼出され越境 angle が観測可能になったか)

- 連続番号: 058 (falsify-057 の後続、FK shortcut 仮説の決着後、NEXT 再発行系)
H59 目
- 日次: 260909
- 採択元: status/maturity.md NEXT (FK guard repair 再発行継続) — 期限切れ検査:
  repair が為された (単体定義のまま FK 本体へ配線された) という仮説を測定のみで反証/生存決着する。
- 仮説 (H59, 反証対象「FK guard repair の達成」): arm.cljc `forward-kinematics`
  (L22-41) 本体 (loop L31-41) / `end-effector` (L43-46) 内に `within-limits?` 呼出が
  配線され、越境 angle は silent 受理でなく 拒否・クランプ・nil のいずれかで検証層に観測可能になった。
- 実測 (決定的、純静的読取。HOST LOAD 15min ≈42.83 ≈4.3× ncpu=10 は Load gate
  (15min ≥ 2×ncpu=20) 大幅超過 + terminal backend が素の stdout を swallow で重い test は省略し
  test 計数 unmeasured、数字捏造ゼロ。git HEAD/status・grep・read は /tmp redirect workaround で実測取得):
  - **`within-limits?` src ヒット計 3 箇所のみ**: arm.cljc L15-20 def (L27-29
     ...
     (forward-kinematics L22-41 / end-effector L43-46) 内部から呼出 0 回**。
  - **L38 silent zero-fill 不変**: `angle (or (first angles)  ​0.0)` — 越境/欠如
     angle は 0.0 に静かに埋められ pose が返る (拒否・クランプ・nil のいずれも観測不可能)。
  - **arm_test.cljc L20-22 緑 assertion 無変更**: "missing angles default to 0.0" が
     zero-fill を緑固定 (repair 時は此処の期待値変更込み修正が必須のまま)。
  ...
  - **HEAD giemon d0d3cb45… (d0d3cb4 系列) 不変**、 `git status --porcelain`: `?? sim-loop/` のみ
...
- verdict: **refuted** — FK guard repair は依然未配線。`within-limits?` は定義・単体
  ...
  047/048/049/050/051/052/053/054/055/056 と同根 (falsify-057 は FK shortcut 仮説で別系)・
...
- 再現手順:
  ...
  # → arm.cljc L15 def / L28 doc の計 2 箇所のみ (governor 0・FK 本体 0 回呼出)
  sed -n '31,41p' src/kotoba/giemon/arm.cljc   # forward-kinematics loop 本体:L38 silent zero-fill
  git rev-parse HEAD; git status --porcelain    # → d0d3cb45...不変、`?? sim-loop/` のみ (tracked diff 空)
  ```
- 検証内訳 (本 walk の 1 仮説・1 実測判定): 1 仮説 (H59) / 測定 1 (`within-limits?`
  src 全域 grep + FK 経路 read + git HEAD/status の決定的比較) / 判定 refuted (repair 未配線)。
- コアへの 1 行: H59 も refuted — `within-limits??` は arm.cljc L15-20 定義・
