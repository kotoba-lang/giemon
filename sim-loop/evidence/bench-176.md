# bench-176 — giemon sim-loop bench (日次・決定的・タイムスタンプなし)

judgement: **measured** — 15-min 負荷帯 ≈ 3.8—3.9× ncpu (実行前 39.48 → 実行後 38.22) で
Load gate 超過帯だったが実行バックエンドは応答し、test スイート実測は完走、
基準値 (bench-066 確定 / bench-175 実測系) と完全一致。回帰なし。

## 実行環境
- git HEAD: giemon `d0d3cb4`、robotics `9459ca0` — 基準値 (bench-066) と同一、不変。
- git diff: 追跡ファイル変更なし (empty)。git status は `?? sim-loop/` のみ未追跡 (bench-175 と同構成)。コード変更なし。
- HOST LOAD: 実行開始前 (pre-run script) 28.90 / 31.99 / 39.48、終了後 34.96 / 31.11 / 38.22
  (15min 39.48→38.22 ≈ 3.8—3.9× ncpu=10)。Load gate は超過帯だが bench-174 (≈9.5×)・
  bench-175 (≈5.4×) より低い帯域。
- 実行バックエンド: terminal 直接 stdout は空のまま (既知 pitfall) だが `/tmp` redirect +
  read_file workaround で test 出力を実測取得 (bench-167〜175 と同手)。load gate
  (15min≥2×ncpu) を超えたが backend 応答・実測完走のため measured 記録
  (skip は backend 非応答のときのみ、bench-108・167・168 前例に従う)。

## テスト（clojure -M:test 実測、/tmp redirect + read_file、一意ファイル b176_*.txt）
...
再現対象の学習ジョブが存在しないため、seeded 再現の本測定は行わない (bench-102〜108・167〜175 と同方針)。
...
falsify-049・bench-175 計測時と同一ソース。純静的 grep 再確認: `within-limits?` のヒットは
arm.cljc L15 (def)・L28 (doc)・arm_test.cljc L29-30 (単体テスト) の計 3 箇所のみで、FK 経路
(forward-kinematics / end-effector) 内呼出は 0 回 (falsify-049・bench-175 と同値、
...
- 本例は 15-min 帯域 ≈ 3.8—3.9× ncpu の Load gate 超過帯だが、bench-169〜175 と同様に
