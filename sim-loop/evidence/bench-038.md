...
## 概要
- cron (giemon-sim-bench)、bench-038
- ホスト負荷: load averages 4.07 (1min) / 7.47 (5min) / 13.82 (15min) 開始
  (ncpu=10 → bench-035/037 級の通常〜やや高め)。テスト実行と軽量 seeded 再現は完了
  (いずれも正常終了)。長時間シミュレータ実行・probe 再実行は **skipped (load)**。
...
- verdict: **pass (軽量範囲)** — bench-002〜037 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜037 の :xf/pos 数値と同一 (決定的再現を 36 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-038 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜038 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 36 点確認
  (本回 bench-038 は load 7.47 (5min)。bench-013 の 184.45 (過去最高) ほか極度高負荷の記録あり)。
- evidence の差分: 本回以降の追加は bench-038.md のみ。
  uncommitted は sim-loop/ ほか (diag.txt, simloop_files_list.txt) のまま。テスト結果には影響なし。

## 4. skipped (load) / 役割外
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **skipped (load)**
  (load 7.47 (5min) は許容範囲だが、テスト・軽量再現以外の重い実験は省略)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench038.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/robotics_test038.txt, /tmp/giemon_test038.txt, /tmp/seed_out038.txt) 経由で
  取得して検証した。テスト結果の数字はファイルから直接読み取ったものである。
- /tmp/seed_parity_bench037.clj (bench-037 由来) を /tmp/seed_parity_bench038.clj に複製して使用
  (内容は bench-002〜037 と同一 — unblob + reconstitute-arm + end-effector 2 回照合)。
