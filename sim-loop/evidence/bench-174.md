# bench-174 — giemon sim-loop bench (日次・決定的・タイムスタンプなし)

judgement: **measured** — 15-min 負荷帯 ≈ 9.5× ncpu (経時最高の 15-min 帯域、bench-173 の
6.7—6.9× を上回る) だったが実行バックエンドは応答し、test スイート実測は完走、
基準値 (bench-066 確定 / bench-167〜173 実測系) と完全一致。回帰なし。

## 実行環境
- git HEAD: giemon `d0d3cb4`、robotics `9459ca0` — 基準値 (bench-066) と同一、不変。
- git diff: 追跡ファイル変更なし (empty)。git status は `?? sim-loop/` のみ未追跡 (bench-173 と同構成)。コード変更なし。
- HOST LOAD: 実行開始前 124.81 / 117.50 / 94.94、終了後 112.42 / 115.01 / 95.99
  (15min 94.94→95.99 ≈ 9.5× ncpu=10)。bench-173 (15min ≈ 6.7—6.9×) を上回る経時最高の
  15-min 帯域だが backend は応答し実測完走。
- 実行バックエンド: terminal 直接 stdout は空のまま (既知 pitfall) だが `/tmp` redirect +
  read_file workaround で test 出力を実測取得 (bench-167〜173 と同手)。load gate
  (15min≥2×ncpu) は大きく超えたが実測 run は成立したため measured 記録
  (skip は backend 非応答のときのみ、bench-108・167・168 前例に従う)。

## テスト（clojure -M:test 実測、/tmp redirect + read_file、一意ファイル b174_*.txt）
...
再現対象の学習ジョブが存在しないため、seeded 再現の本測定は行わない (bench-102〜108・167〜173 と同方針)。
...
falsify-049・bench-173 計測時と同一ソース。純静的 grep 再確認: `within-limits?` のヒットは
arm.cljc L15 (def)・L28 (doc)・arm_test.cljc L29-30 (単体テスト) の計 3 箇所のみで、FK 経路
(forward-kinematics / end-effector) 内呼出は 0 回 (falsify-049・bench-167〜173 と同値、
FK guard repair 未着手のまま)。既存 falsify-001〜049 は全て決着 (maturity 正本: falsify-024〜033/
036〜038 refuted、039〜049 各決着)、残存 pending なし。本 walk の新決着なし。
...
- 本例は 15-min 帯域 ≈ 9.5× ncpu と経時最高だが、bench-171 (3.7×)・bench-172 (5.4×)・
  bench-173 (6.7—6.9×) と同様に backend 応答・実測完走・基準値一致で measured。skip 判定
  (backend 非応答) には至っていない点を正直に明記。