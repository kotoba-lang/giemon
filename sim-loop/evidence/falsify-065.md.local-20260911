# falsify-065 (H66) — FK guard repair 未配線 (27 連続 refuted)
...
- 実測 (純静的読取。git/grep/sed は /tmp redirect workaround で取得、数字捏造ゼロ。
  出力全文 /tmp/falsify065.txt):
  - HEAD giemon `d0d3cb45fcc8c42d94f6a370b5a1f19d51938abe` 不変
    (falsify-062/063/064 と同一 commit)。`git status --porcelain`: `?? sim-loop/` のみ、
  ...
  - FK 経路 (`forward-kinematics` L22-41 / `end-effector` L43-46) 本体内部からの
    ...
    が loop 本体 (L30) に不変で残存 — 越境/欠損 angles を 0.0 で silent 受理し pose を返す。
...
- verdict: **refuted** — FK guard repair は依然未配線。越境 angles は silent 受理で
  pose 返却。falsify-034/036/039〜064 と同根 (falsify-057 は別系)・**27 連続 refuted**。
- HOST LOAD: 開始時 13.27/17.26/22.09 → 測定中も同値 — 15-min ~22.09 が ncpu=10 の
  Load gate (15min ≥ 2×ncpu=20) 超過 (~2.2×) → 重い test 実行・seeded 再現は省略し
...
- 再現手順:
  ...
  grep -cE "arm|limit|torque|joint" src/kotoba/giemon/governor.cljc  # 0
  ```
- コアへの 1 行: guard は 27 iterations 丸ごと未配線 — `within-limits?` を FK 経路に
