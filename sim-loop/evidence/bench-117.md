...
判定: **measured (完走)** — HOST LOAD が ncpu=10 の ~1.5× (15min 14.96) で clojure -M:test 実測完走。robotics・giemon とも基準値一致、回帰なし。
...
##再現コマンド
- 実行: `cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/robotics && clojure -M:test > /tmp/b_rob.txt 2>&1`; `cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M:test > /tmp/b_gie.txt 2>&1` —許 完走 (measured,回帰なし)。/tmp/b_rob.txt / /tmp/b_gie.txt に実測保存。
