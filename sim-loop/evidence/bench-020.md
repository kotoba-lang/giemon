...
## 概要
- cron (giemon-sim-bench)、bench-020
- ホスト負荷: load averages 18.42 (1min) / 22.09 (5min) / 24.19 (15min) 開始 (ncpu=10 →
  **中負荷**)。重い実験 (数値計測 probe の新規追加・長時間シミュレータ実行) は
...
- verdict: **pass (軽量範囲)** — bench-002〜019 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜019 の :xf/pos 数値と同一 (決定的再現を 18 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-020 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜020 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 18 点確認
  (本回 bench-020 は load 22.09 (5min)。bench-019 の 22.65 と同程度、bench-007 の 37.25 を下回る)。
- evidence の差分: 前回 (bench-019) 以降、evidence への追加・変更なし
  (probe_offdiag_inertia.py は falsify-008 用 probe として既に同梱済み・falsify-008 実行は本回未実施)。
  uncommitted は sim-loop/ のまま。テスト結果には影響なし。

## 4. skipped (load)
- 重い実験 (off-diagonal inertia の反証 falsify-008 の実行を含む) は実施せず
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench020.clj
...
- 本回も terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/giemon_test_020.txt, /tmp/robotics_test_020.txt, /tmp/bench020_host.txt,
  /tmp/bench020_parity.log, /tmp/bench020_seedgrep.txt) 経由で取得して検証した。
