...
## 概要
- 実行日: 2026-09-05 (JST) — cron (giemon-sim-bench)、bench-004
- ホスト負荷: load averages 8.22–18.65 (1min) / 18.04–31.38 (15min)、ncpu=10 → **高負荷**
...
- verdict: **pass (軽量範囲)** — HOST LOAD 高負荷のため重い実験は skipped (load)、
  bench-002/003 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果 (:xf/pos [0.035165560086391295 0.0 0.6104766433183229] /
    ...
    bench-002/003 の :xf/pos 数値と同一 (決定的再現を 3 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → bench-002 → bench-003 → bench-004 で同一数字:
