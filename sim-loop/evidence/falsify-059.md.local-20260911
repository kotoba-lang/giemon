# falsify-059 — H60 (FK guard repair が配線されたか: `within-limits?` が FK 経路内から呼出され越境 angle が観測可能になったか)

- 連続番号: 059 (falsify-058 の後続、FK shortcut 仮説の決着後、NEXT 再発行系)

  H60 目
- 日次: 260909
- 採択元: status/maturity.md NEXT (FK guard repair 再発行継続) — 期限切れ検査:
  repair が為された (単体定義のまま FK 本体へ配線された) という仮説を測定のみで反証/生存決着する。
- 仮説 (H60, 反証対象「FK guard repair の達成」): arm.cljc `forward-kinematics`
...
- 実測 (決定的、純静的読取。HOST LOAD ≈39.94 (≈4.0× ncpu=10) は Load gate
   (15min ≥  ̃2×ncpu=20) 大幅超過 + terminal backend が素の stdout を swallow で重い test は省略し
  ...
  - **arm_test.cljc L20-22 緑 assertion 無変更**: "missing angles default to 0.0" が
     silent zero-fill を緑固定 (repair 時は此処の期待値変更込み修正が必須のまま)。
...
- verdict: **refuted** — FK guard repair は依然未配線。`within-limits?` は定義・単体
  ...
  047/048/049/050/051/052/053/054/055/056/058 と同根 (falsify-057 は FK shortcut 仮説で別系)、
...
- コアへの 1 行: H60 も refuted — `within-limits?` は arm.cljc L15-20 定義・
