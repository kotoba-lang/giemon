# falsify-051 — H52 (FK guard repair が着手され、FK 経路は越境 angles を検査するか)

- 連続番号: 051 (falsify-050 の後続)
- 日次: 260908
- 仮説 (H52, NEXT 継続発行 — falsify-034/036/039/040/041/042/043/044/045/046/047/048/049/050 の 15 連続 refuted を受けた再検証):
...
- 実測 (決定的、純静的読取 — HOST LOAD 大幅超過帯 + terminal backend が素の stdout を swallow し
  ...
  - **`within-limits?` 定義/呼出分布 (grep src/ test/ 実測):** arm.cljc L15 定義・arm.cljc L28
    docstring 言及・arm_test.cljc L29-30 単体テストのみの計 5 ヒット (3 箇所)。FK 経路
  ...
  - **governor 接続:** governor.cljc を grep `within-limits|:joint/limit|torque|forward-kinematics|
    end-effector` 実測は **0 行** — arm limit/torque/within-limits に無接続のまま。
  ...
  - 測定時 HOST LOAD: 本 walk 冒頭 19:04 実測 34.43/42.09/64.70 (1min/5min/15min、≈3.3–6.5×
    ...
    据え置き)。
- verdict: **refuted** — FK guard repair は依然未着手 (16 連続)。`within-limits?` は定義・doc・
  ...
  **0 回**、L38 silent zero-fill (`angle (or (first angles) 0.0)`) 不変、arm_test L20-22 が零埋め
  ...
  返却 (docstring L27-29 自白)。falsify-034/036/039/040/041/042/043/044/045/046/047/048/049/050
...
- 検証内訳 (1 仮説・1 実測判定): 1 仮説 (H52) / 測定 1 (純静的読取 — within-limits? grep 分布
  ...
  判定 refuted (未着手)。
- コアへの 1 行: FK guard repair は 16 連続で未着手のまま (`within-limits?` は定義・単体テストのみ
  で FK 内呼出 0 回、L38 silent zero-fill は無変更、governor は arm に無接続、HEAD d0d3cb4 不変 —
  越境 angles を silent 受理継続)、本 bot は実装・変更なし (HOST LOAD 34.43/42.09/64.70
  ≈3.3–6.5× 大幅超過帯で重い test 実行を省略し unmeasured 明記)。