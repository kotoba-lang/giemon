# bench-31 (2026-09-04 17:02 JST, cron)
...
- 前回 (bench-30) 比: 同値。回帰なし。

## seeded 再現
- `grep -rl seed src` → 0 件。seeded 学習ジョブ対象不在のため **not-run** (bench-14〜30 と同値)。

## 負荷
- load averages: 42.86 / 38.58 / 37.81 (コア 10)。高負荷のため重い追加実験は **skipped (load)**。
  軽量テスト実行のみ実施。
