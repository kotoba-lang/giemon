# bench-179 — giemon sim-loop bench (日次・決定的・タイムスタンプなし)

judgement: **measured** — 15-min 負荷帯 ≈ 5.1× ncpu (実行開始前 52.62 → 終了後 50.65)
の Load gate 超過帯だったが、実行バックエンドは応答し、両 test スイート実測は
完走、基準値 (bench-066 確定 / bench-178 実測系) と完全一致。回帰なし。

## 実行環境
- git HEAD: giemon `d0d3cb4`、robotics `9459ca0` — 基準値 (bench-066) と同一、不変。
- git diff: 追跡ファイル変更なし (empty、DIFF_RC=0)。git status は `?? sim-loop/` のみ未追跡 (bench-177/178 と同構成)。コード変更なし。
- HOST LOAD: 実行開始前 (pre-run script) 36.91 / 36.28 / 52.62、終了後 34.82 / 35.67 / 50.65
  (15min 52.62→50.65 ≈ 5.1× ncpu=10)。Load gate (15min≥2×ncpu ≈ 20) に対し超過帯。
- 実行バックエンド: terminal 直接 stdout は空のまま (既知 pitfall) だが `/tmp` redirect +
  read_file workaround で test 出力を実測取得。load gate 超過にもかかわらず
  backend は応答・実測完走したため measured 記録 (skip は backend 非応答のときのみ、
  bench-108・167・175〜178 前例に従う)。

## テスト（clojure -M:test 実測、/tmp redirect + read_file、一意ファイル b_rob.txt / b_gie.txt）
...
再現対象の学習ジョブが存在しないため、seeded 再現の本測定は行わない (bench-167〜178 と同方針)。
...
コアは falsify-050・bench-178 計測時と同一ソース。純静的再確認: `within-limits?` のヒットは
arm.cljc L15 (def)・L28 (doc)・arm_test.cljc L29-30 (単体テスト) の計 3 箇所のみで、FK 経路
(forward-kinematics / end-effector) 内呼出は 0 回 (falsify-050 〜 bench-178 と同じ、FK guard
...
- 本例は 15-min 帯域 ≈ 5.1× ncpu の Load gate 超過帯だったが、backend 応答・実測完走・
