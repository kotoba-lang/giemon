...
## 概要
- cron (giemon-sim-bench)、bench-014
- ホスト負荷: load averages 36.09 / 60.43 / 62.35 開始時 → 実行中 45.10 / 60.25 / 62.21、
  ncpu=10 → **高負荷** (bench-013 の 184.45 よりは低いが bench-007 の 37.25 相当)。
...
- verdict: **pass (軽量範囲)** — bench-002〜013 と同一の軽量 seeded 再現のみ実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (src/kotoba/giemon/ は giemon/chassis/export/governor/kinematics/ui/viewer/
  arm のみ。本回は src + test 全 .clj/.cljc 18 ファイルを列挙し、seed/learn/train を
  含むファイルがないことを grep (exit 1, 一致なし) で機械確認)。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  - 対象: `kotoba.giemon.arm/end-effector` (giemon_arm6 fixture —
    ...
    (unblob + reconstitute) で再構成、q=[0.0 0.2 -0.3 0.0 0.5 0.0])
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    :xf/pos [0.035165560086391295 0.0 0.6104766433183229]
    bench-002〜013 の :xf/pos 数値と同一 (決定的再現を 13 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-014 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜014 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 12 点確認
  (本回 bench-014 は load 60.43 (5min)。bench-007 の 37.25 を上回る負荷での同一結果)。
- evidence の差分: 前回 (bench-013) 以降、evidence への追加・変更なし
