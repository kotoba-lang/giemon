...
- date-run: 2026-09-13 22:08 JST (決定的・タイムスタンプは記録のみ)
- host load: `up 8 days, 14:51, load averages: 10.76 19.06 20.87`, hw.ncpu=10
  → 15-min ≈ 1.9x < 2x gate, therefore ran (not skipped).
...
装って何も測っていない。maturity.md NEXT の test-runner 修復 (cljk rename 後の
silent-zero, kbb -M:test RC=1) は **解消されていない**。base 値との比較は不能、
regression assert はしない (honest: unmeasured-equivalent)。
