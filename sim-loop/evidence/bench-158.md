# bench-158 — giemon sim-loop ベンチ (決定的・タイムスタンプなし)

judgement: **unmeasured** (load skip — 15min 60.34 ≈ 6.03x ncpu=10、2x gate 大幅超過)
load: 1min 110.01 / 5min 86.95 / 15min 60.34, ncpu=10 (uptime 13:21)
host: macOS, 7 users

## テスト
- 未実行 (load gate skip)。15-min load 60.34 ≧ 2×ncpu=20 で重い test スイート
  の実測を省略。honest に `unmeasured` と記録し、回帰は assert しない
  (基準値据え置き — bench-157 と同方針)。これは本 walk 最大但し書き: 負荷帯が
  前回 (15min 38.98) を更に上回り、重い実測は実行バックエンド応答不能リスク大。
