# falsify-052 — H53 (FK guard repair が着手され、FK 経路は越境 angles を検査するか)

- 連続番号: 052 (falsify-051 の後続)
- 日次: 260908
- 仮説 (H53, NEXT 継続発行 — falsify-034/036/039/040/041/042/043/044/045/046/047/048/049/050/051
  の 16 連続 refuted を受けた再検証):
...
- 実測 (決定的、純静的読取 — HOST LOAD 大幅超過帯 + terminal backend が素の stdout を swallow し
  ...
  - **governor 接続:** governor.cljc を grep `within-limits` 実測は **0 行** — arm
    limit/torque/within-limits に無接続のまま。
  ...
  - 測定時 HOST LOAD: 本 walk 冒頭 20:16 実測 39.38/42.87/40.32 (1min/5min/15min、≈3.9–4.3×
    ncpu=10)。Load gate (15min ≥ 2×ncpu=20) を大幅超過 + terminal backend 応答不能 (素のコマンド
    出力が逐次 swallow、redirect 経由で取得) — 重い test 実行は省略し unmeasured (honest
    据え置き)。
- verdict: **refuted** — FK guard repair は依然未着手 (17 連続)。`within-limits?` は定義・doc・
  ...
  **0 回**、L38 silent zero-fill (`angle (or (first angles) 0.0)`) 不変、arm_test L20-22 が零埋め
  ...
  返却 (docstring L27-29 自白)。falsify-034/036/039/040/041/042/043/044/045/046/047/048/049/050/051
...
- 再現手順 (全て静的読取、実行不要):
  ...
  grep -c 'within-limits' src/kotoba/giemon/governor.cljc # → 0 (無接続)
  ```
- 検証内訳 (1 仮説・1 実測判定): 1 仮説 (H53) / 測定 1 (純静的読取 — within-limits? grep 分布
  (FK 内呼出 0) + L38 zero-fill 不変 + governor 接続 0 + HEAD/tracked diff の 4 観測点) /
  判定 refuted (未着手)。
- コアへの 1 行: FK guard repair は 17 連続で未着手のまま (`within-limits?` は定義・単体テストのみ
  で FK 内呼出 0 回、L38 silent zero-fill は無変更、governor は arm に無接続、HEAD d0d3cb4 不変 —
  越境 angles を silent 受理継続)、本 bot は実装・変更なし (HOST LOAD 39.38/42.87/40.32
  ≈3.9–4.3× 大幅超過帯で重い test 実行を省略し unmeasured 明記)。