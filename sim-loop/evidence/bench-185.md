# bench-185 — unmeasured (host load)

判定: **unmeasured** (HOST LOAD 超過により clojure test / seeded 再現をスキップ、基準値据え置き・回帰 assert なし)

## 環境
- HOST LOAD (15-min) ≈ 36.1 (5-min ≈ 35.5、1-min ≈ 22.7)
- hw.ncpu = 10  → 15-min load ≈ 3.6× ncpu ≧ 2× gate → 重い実行は省略 (負荷帯が非応答域)
...
## 備考
- bench-185 は bench-099〜101 / bench-182 / bench-183 / bench-184 と同様の load-skip 判断。基準値は bench-066 確定
  (robotics 14/50/0、giemon 46/115/0)。
- コード変更なし (`?? sim-loop/` のみ untracked、bench-184 から実質変化なし)。
- ホスト負荷 (15-min ≈ 36.1) は bench-184 (≈38.4) よりやや低下したが、依然 3.6× ncpu で非応答域を継続。