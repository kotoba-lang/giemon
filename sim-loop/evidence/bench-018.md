...
## 概要
- cron (giemon-sim-bench)、bench-018
- ホスト負荷: load averages 20.30 (1min) / 27.24 (5min) / 26.26 (15min) 開始、ncpu=10 →
  **中〜高負荷**。重い実験 (数値計測 probe の新規追加・長時間シミュレータ実行) は
...
- verdict: **pass (軽量範囲)** — bench-002〜017 と同一の軽量 seeded 再現のみ実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま**。L1 昇格条件は未達、OPEN の赤は解消されず。
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜017 の :xf/pos 数値と同一 (決定的再現を 16 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-018 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜018 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 16 点確認
  (本回 bench-018 は load 27.24 (5min)。bench-007 の 37.25 を下回る負荷での同一結果)。
- evidence の差分: 前回 (bench-017) 以降、evidence への追加・変更なし
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench018.clj
...
- 本回も terminal ツールの stdout が断続的に空になる障害があり、コマンド出力を
  一時ファイル (/tmp/robotics_test.log, /tmp/giemon_test.log, /tmp/bench018_host.txt,
  /tmp/bench018_parity.log) 経由で取得して検証した。テスト結果の数字はファイルから
  直接読み取ったものである。
