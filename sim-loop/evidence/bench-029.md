...
## 概要
- cron (giemon-sim-bench)、bench-029
- ホスト負荷: load averages 89.59 (1min) / 86.79 (5min) / 67.49 (15min) 開始
...
- verdict: **pass (軽量範囲)** — bench-002〜028 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `SEED-PARITY true` (exit 0)、
    ...
    bench-002〜028 の :xf/pos 数値と同一 (決定的再現を 27 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-029 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜029 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 27 点確認
  (本回 bench-029 は load 86.79 (5min)。bench-013 の 184.45、bench-023 の 99.45 に次ぐ
  過去 3 番目の高水準でも同一)。
- evidence の差分: bench-028.md が前回実行で追加済み。本回以降の追加は bench-029.md のみ。
  uncommitted は sim-loop/ ほか (diag.txt, simloop_files_list.txt) のまま。テスト結果には影響なし。

## 4. skipped (load) / 役割外
- 長時間シミュレータ実行・probe (falsify 系列) の再実行は **skipped (load)**
  (load 86.79 (5min) はテスト・軽量再現は可能だが重い実験は省略する水準)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench029.clj
...
- terminal ツールの stdout が空になる障害が継続しており、コマンド出力を
  一時ファイル (/tmp/giemon_test029.txt, /tmp/robotics_test029.txt, /tmp/seed_out029.txt) 経由で
  取得して検証した。テスト結果の数字はファイルから直接読み取ったものである。
- /tmp/seed_parity_bench028.clj を /tmp/seed_parity_bench029.clj に複製して使用
  (内容は bench-024〜028 と同一 — unblob + reconstitute-arm + end-effector 2 回照合)。
