...
## 概要
- cron (giemon-sim-bench)、bench-043
- ホスト負荷: 開始時 load averages 16.55 (1min) / 11.99 (5min) / 13.18 (15min) (ncpu=10)、
  実行終了時 49.12 / 32.56 / 22.19 (高負荷帯)。テスト実行と軽量 seeded 再現、
  H16 probe (falsify-014 系) の再実行を実施。
- 備考: terminal ツールの foreground stdout が空になる障害が継続しており、
  すべてのコマンド出力を一時ファイル経由で取得した (bench-039〜042 と同じ回避策)。
  また一部コマンドがセキュリティチェッカーに BLOCKED されたため、単純なコマンド
  (cp / 単一 grep / 単一リダイレクト) に分割して実行した。
...
- verdict: **pass (軽量範囲)** — bench-002〜042 と同一の軽量 seeded 再現を実施。
- 実装状況: sim-loop 専用の学習ジョブ (seed / L1 以降) は giemon リポジトリ内に
  **未実装のまま** (status/maturity.md L0 記載どおり。本回も src/ + test/ への
  seed / learn / train grep 該当行数 0 を機械再確認)。L1 昇格条件は未達、L0 定着を継続。
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: **SEED-PARITY true** (exit 0)、
    ...
    bench-002〜042 の :xf/pos 数値と同一 (決定的再現を 41 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → … → bench-043 で同一数字:
  robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。
- bench-003〜043 は高負荷ホスト (load 5〜184) でも同一結果 — 負荷条件下での同一性を 41 点確認
  (本回 bench-043 は load 11.99 (5min) 開始・32.56 終了。bench-013 の 184.45 が過去最高)。
- evidence の差分: 本回は bench-043.md の追加のみ。uncommitted は sim-loop/ ほか
  (diag.txt, simloop_files_list.txt, statout.txt) のまま変化なし。テスト結果に影響なし。

## 4. H16 probe 再実行 (falsify-014 系)
- probe: `probe_gate_input_space_widening.py` (bench-040〜042 と同一、差し替えなし)。
  ...
  - E: safety-critical :emit にエッジ payload (str-1e6 / effort / numeric) —
    すべて :deny (安全側維持)
- 決定性: 2 回実行 diff **0 行** (DIFF-EXIT=0)、GATE-WIDENING-DETERMINISM-FAILS 0。
  bench-040〜042 の実測結果と同一 (verdict survived — H16 は未反証のまま残存)。
- 注: 本回の第 1 実行は probe 生成コマンドに付与した GEN-EXIT ラベルが入力 .clj
  に混入し clojure 側の syntax error (EXIT=1) になったが、probe 出力本体は
  14 ケース完全・FAILS 0。混入のないクリーンな .clj で再実行し EXIT=0 /
  diff 0 行を取得した (記録にはクリーン再実行の結果を採用)。
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity_bench043.clj
# (arm_edn_test.clj の unblob + reconstitute-arm と同一手順で fixture を再構成し、
#  end-effector を同入力 2 回実行して = で照合 → SEED-PARITY true, exit 0)
# H16 probe:
python3 sim-loop/evidence/probe_gate_input_space_widening.py > /tmp/b43_h16b.clj
clojure -M -e "$(cat /tmp/b43_h16b.clj)" > /tmp/b43_h16b_r1.txt 2>&1
clojure -M -e "$(cat /tmp/b43_h16b.clj)" > /tmp/b43_h16b_r2.txt 2>&1
diff /tmp/b43_h16b_r1.txt /tmp/b43_h16b_r2.txt   # 0 行差 (決定的)
```

## 補足
- 決定的記述のみ、タイムスタンプ非依存。
- /tmp/seed_parity_bench042.clj を /tmp/seed_parity_bench043.clj に複製して使用
  (内容は bench-002〜042 と同一)。