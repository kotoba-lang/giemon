...
## 概要
- 実行日: 2026-09-05 (JST) — cron (giemon-sim-bench)、bench-006
- ホスト負荷: load averages 8.55 (1min) / 12.49 (5min) / 9.97 (15min)、ncpu=10 → **高負荷**
...
- verdict: **pass (軽量範囲)** — HOST LOAD 高負荷のため重い実験は skipped (load)、
  bench-002〜005 と同一の軽量 seeded 再現のみ実施。
...
- 軽量 seeded 再現 (決定的関数の同入力 2 回実行照合):
  ...
  - 結果: 2 回の結果が**完全一致** → `PARITY true` (exit 0)、
    ...
    bench-002/003/004/005 の :xf/pos 数値と同一 (決定的再現を 5 点確認)。
- 注: 学習ジョブの seeded 再現ではなく、既存決定的関数の 2 回実行一致の確認。

## 3. 回帰の有無
- **回帰なし**。bench-001 → 002 → 003 → 004 → 005 → 006 で同一数字:
...
# seeded 再現 (軽量):
cd /Users/junkawasaki/github/com-junkawasaki/orgs/kotoba-lang/giemon && clojure -M -i /tmp/seed_parity.clj
