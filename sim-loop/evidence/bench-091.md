# bench-091 — SKIPPED (load & unresponsive backend)
...
## 理由
- HOST LOAD (pre-run 時点): 17.41 / 23.19 / 31.78 (1min/5min/15min), ncpu=10
  → 実行バックエンド応答不能のため重い実験 (test スイート・学習再現) は実行不能。
- 実行バックエンド応答不能: terminal が空出力 (exit 0) を返すまま応答せず
  (`echo alive` / `uptime` / `date` / `pwd` すべて空)。search_files は
  `could not stat .../giemon (sandbox may still be starting or was removed)` を返し
  evidence/sandbox stat 不能。
  → bench-069〜090 と同じ応答不能条件。write_file / read_file のみ成立。
...
- falsify-001〜029 (H1〜H31 refuted 閉) は prior 記録のまま（本イテレーション新規 falsify なし）。
  bench-089/090 の結論どおり falsify 面 (src 層 H26-H30 + test 層 H31) は網羅的に閉じたため
  新規 void-falsify 候補なし。
- NEXT: FK/end-effector の numeric guard (NaN/±∞, H29/H30) + shape guard (長さ, H26/H27/H28)
  の導入判断（コード修正 + test 追加を伴う、falsify-029/H31 で test surface も無防備と確定）
  はコア (giemon-sim) 側の実施対象であり本 bot は実施しない。本 bot の残る測定面は
  backend 復旧後の test スイート + seeded 再現による基準値再確認 (bench-092 で実施予定) —
  L0 昇格条件 (学習ジョブ実装) と基準値再検証の両方が復旧待ち。
