...
集計元: sim-loop/evidence/ bench-001〜067 / falsify-001〜023 (決定的計算、
bench-064〜065 は load 超過 (1min 188 / 92〜116)、bench-067 は load 1min 74〜99 持続・
上昇でテスト再実行を skipped (honesty-first)、基準値保持の判定は bench-066 で確定済み)
...
|---|---|---|---|
| 1 | テスト健全性 | 緑 | robotics 14 tests / 50 assertions / 0 failures、giemon 46 tests / 115 assertions / 0 failures。bench-001→066 で同一数字 (回帰 0、負荷 5〜180+ でも同一性 64 点確認 — bench-059 は test 時 load 10〜20 / 1min 12、bench-060 は load 7.16〜8.16 / 5min 9.2 / 15min 12.1、bench-061 は load 9.43 / 5min 7.92 / 15min 9.2、bench-062 は load 12.18 / 5min 10.27 / 15min 9.75、bench-063 は load 31.77 / 5min 32.04 / 15min 24.43 の高負荷、bench-056 はテスト前後 load 92〜180 でも同一、bench-066 は開始時 1min 59.75 / 5min 56.97 / 15min 79.07 → 終了時 84.34 / 76.18 / 79.33 (ncpu=10) の高負荷で 2 スイートとも exit 0)。bench-064〜065 は load 超過 (188 / 92〜116)、bench-067 は load 1min 74〜99 持続・上昇 (5 秒間隔でも 92.70 → 99.45) で test スイート実行を skipped (honesty-first、決定的数字を捏造せず skipped 記録。基準値保持の判定は bench-066 再検証で回帰なしを確定) |
| 2 | 決定性 / seeded 再現 | 黄 | SEED-PARITY true を bench-002〜066 で 63 点確認 (:xf/pos 同一、bench-066 で再確認 — 62 点目は bench-063、bench-066 で 63 点目一致)。bench-064〜065/067 は load 超過で seeded 再現未実行 (skip のため点カウント外)。ただし学習ジョブ不在につき「既存決定関数の 2 回実行一致」に限る — 減点事由。falsify-016: 姿勢付き surrogate も 16/16 決定的。falsify-017 により normalize アンダーフロー域での潜在赤あり (現 fixture 軸は安全) |
...
- NEXT: H26 -- 「FK は角度列と chain 長の不一致を検証しない」を測る。
  `forward-kinematics` は angles 列が chain より短い (欠落) とき
  `(or (first angles) 0.0)` でゼロ充填、長いとき末尾を無視する (arm.cljc 再読:
  `angle (or (first angles) 0.0)`, `(rest angles)` -- 長さ一致を検査する分岐が無い)。
  input 空間 probe (角度列を 0 個 / 1 個欠らした / chain より 1 個長い / 型混在を含む
  の 4 種を FK に渡し、戻り `:xf/pos` がゼロ充填値のまま通る・例外が無い・形状不一致を
  拒否しない) で「角度列形状検証面の不在」を決定的に測る。
  (コード修正はコア (giemon-sim) 側の対象であり本 bot は実施しない。
  falsify-024 として evidence に記録するまで next を進めない。)
